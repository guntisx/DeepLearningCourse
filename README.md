...
These three mysteries form a minimal causal chain for true AI: open-ended learning must get its data from the physical world; perception must compress that data into persistent objects, concepts, and instances; and language-based reasoning systems must acquire lifelong memory over those grounded symbols. Evolution shows how data can be generated by embodied agents in competition, while LLMs show the power of symbolic abstraction—but the missing bridge is grounded, persistent, one-shot conceptual memory.
